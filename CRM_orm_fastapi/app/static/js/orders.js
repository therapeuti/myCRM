...
if (parameters.get('ordertime')) {
    orderdate.value = parameters.get('ordertime')}
    ...
    for (let key of ths) {
        ...
        switch (key) {
            case 'id':
                ...
                new_a.href = `/order/info/${order[key]}`
            ...
            case 'store_id':
                ...
                new_as.href = `/store/info/${order[key]}`
            ...
            case 'user_id':
                ...
                new_au.href = `/user/info/${order[key]}`
...
function fetch_orders_data() {
    ...
    return fetch(`/api/v1/orders/?${query.toString()}`) // blueprint 사용시 fetch경로
        .then(response => {
